...
#define geraet11 //TTGO MINI EPAPER CORE 1.02" // WETTER 02 in Wildwetter
...
// include the library
  ...
  GxEPD2_BW<GxEPD2_102, GxEPD2_102::HEIGHT> display(GxEPD2_102(/*CS=5*/ E_INK_PIN_SPI_CS, /*DC=*/ E_INK_PIN_SPI_DC, /*RST=*/ E_INK_PIN_SPI_RST, /*BUSY=*/ E_INK_PIN_SPI_BUSY)); // GxEPD2_102
  ...
  boolean LargeIcon = true, SmallIcon = false; //250x122 Pixel
  const uint16_t window_x = 0;
  const uint16_t window_y = 0;
  const uint16_t window_width = 128;
  const uint16_t window_height = 80;
#endif
//Led Einstellungen
...
void setup() {
    ...
    #ifdef EINKDISPLAY102TTGO
      display.epd2.init(E_INK_PIN_SPI_SCK, E_INK_PIN_SPI_DIN, 115200, true, 20, false); // define or replace SW_SCK, SW_MOSI
      display.init(115200);
...
void LEDS_gruen_oben(void){
  ...
  int count_decoded_wind_messages =0;*/
      void displayWeatherData(void){
                display.fillScreen(GxEPD_WHITE);
                display.setTextSize(1);
                display.setCursor(0,15); display.print("Regen:"); display.setCursor(60,15);display.print(rain_cc1101); 
                display.setCursor(0,30); display.print("Wind:"); display.setCursor(50,30);display.print(wind_speed_cc1101); 
                display.setCursor(0,45); display.print("Richtung:"); display.setCursor(90,45); display.print(wind_dir_cc1101);      
                display.setCursor(0,60); display.print("Temp:"); display.setCursor(50,60); display.print(temp_cc1101);      
                display.setCursor(0,75); display.print("Hum:"); display.setCursor(70,75); display.print(humidity_cc1101);      
                while (display.nextPage());
        }
        void clearWindow(){
